#!/bin/bash
GLIB_ELF_CROSS_PREFIX=/opt/gcc-riscv64-unknown-linux-gnu/bin/riscv64-unknown-linux-gnu

run_linux()
{
    LOOPDEV="$(losetup -f)"
    losetup -o 0 --sizelimit 1073741824 $LOOPDEV $1/rootfs.img -P
    if [ -d "$1/target" ]; then  
    rm -rf $1/target
    fi
    ...
    cp -p -r $1/rootfs/* $1/target/rootfs/
    sync
    echo "please wait 5s"
    sleep 5
