...
static ssize_t edu_write(struct file *filp, const char __user *buf,
    ...
    // Get edu_device
    edu_dev = filp->private_data;
    printk(KERN_INFO"[SYJ ADD %s] COUNT=%d PPOS=%d \n",
                       DRIVER_NAME,count,*ppos);
    // Read data first
    if (count == 4) {
    ...
    printk(KERN_INFO"[SYJ ADD %s] value32=%d \n",
                       DRIVER_NAME,value32);
...
static int edu_probe(struct pci_dev *pdev, const struct pci_device_id *id) {
    ...
    if ((ret = dma_set_mask_and_coherent(&(pdev->dev), DMA_BIT_MASK(32))) < 0) {
        dev_warn(&pdev->dev, "[%s] No suitable DMA available\n", DRIVER_NAME);
        goto release_region;
    }
    printk(KERN_ERR "[%s] succee to pci create device node\n", DRIVER_NAME);
   ...
   edu_dev->complete = false;
   iowrite32(value32_s, edu_dev->mmio_base + EDU_FACT_CALC);
   msleep(500);
   //value32 = ioread32(edu_dev->mmio_base + EDU_FACT_CALC);
   ret = wait_event_interruptible(edu_dev->wait_queue,
                                           edu_dev->complete);
            // Deal with signal interruptions
            if (ret == -ERESTARTSYS) {
                printk(KERN_INFO "[%s] error no finish \n", DRIVER_NAME);
            }
            // Read data
            value32_s = ioread32(edu_dev->mmio_base + EDU_FACT_CALC);
